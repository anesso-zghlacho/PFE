...
class PacketAnalysisService:
    ...
    def analyze_and_store(self, packet: dict) -> tuple[InferenceResult, TrafficLog, Optional[Alert]]:
        ...
        traffic_log = TrafficLog.objects.create(
            ...
            packets_per_sec=1.0,
            syn_count=1 if 'SYN' in str(result.features.get('tcp_flags', '')).upper() else 0,
            ack_count=1 if 'ACK' in str(result.features.get('tcp_flags', '')).upper() else 0,
            fin_count=1 if 'FIN' in str(result.features.get('tcp_flags', '')).upper() else 0,
