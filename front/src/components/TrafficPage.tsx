          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-card z-10">
              <tr className="border-b border-border bg-secondary/50">
                ...
                <th className="text-left px-3 py-2.5 text-muted-foreground font-medium text-xs">Flags</th>
                <th className="text-left px-3 py-2.5 text-muted-foreground font-medium text-xs">Size</th>
              </tr>
            </thead>
            <tbody>
              {trafficLogs.slice(0, 100).map(log => (
                <tr key={log.id} className="border-b border-border/30 hover:bg-secondary/20 transition-colors">
                  <td className="px-3 py-2 font-mono text-[11px] text-muted-foreground whitespace-nowrap">
                    {log.timestamp.toLocaleTimeString('en', { hour: '2-digit', minute: '2-digit', second: '2-digit', fractionalSecondDigits: 1 } as Intl.DateTimeFormatOptions)}
                  </td>
                  ...
                  <td className="px-3 py-2">
                    {log.flags.length > 0 ? (
                      <div className="flex gap-1">
                        {log.flags.map(f => (
                          <span key={f} className="font-mono text-[10px] bg-secondary text-secondary-foreground px-1.5 py-0.5 rounded">
                            {f}
                          </span>
                        ))}
                      </div>
